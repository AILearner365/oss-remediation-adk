...
Material decisions are recorded as complete current snapshots. The runtime separately reports `decisionCaptureStatus` as `COMPLETE`, `LATE`, `INCOMPLETE`, or `MISSING` in each cycle artifact and `final-result.json`. `COMPLETE` requires an initial `SELECT` before the first observed workspace-edit capability mutation and a current terminal `READY_FOR_INDEPENDENT_VALIDATION` or `BLOCK` snapshot before validation. A retroactive selection remains `LATE`; a nonterminal or stale chain is `INCOMPLETE`; no accepted record is `MISSING`. This ordering currently observes `edit_workspace_text` precisely. Shell commands may mutate repository files but cannot yet be classified precisely, so artifacts explicitly report shell mutation detection as unavailable rather than claiming full process-level mutation coverage.

When capture is deficient after a work turn, the same session receives one bounded metadata-only reconciliation opportunity before deterministic validation. During that turn every operational capability is blocked and only `record_decision` is available; it consumes no operational tool-call budget and cannot make a late record historically timely. Deterministic validation remains authoritative regardless of capture quality.
...
A deterministically validated remediation with `LATE`, `INCOMPLETE`, or `MISSING` decision capture is preserved as validated but receives `VALIDATED_DECISION_AUDIT_REVIEW_REQUIRED`; automatic push/PR delivery is withheld. `COMPLETE` capture may proceed through the normal delivery eligibility checks. Newly introduced untracked dependency-tree diagnostic reports are also surfaced in validation evidence and withhold automatic delivery until reviewed or removed; deterministic remediation success is not reclassified as failure.

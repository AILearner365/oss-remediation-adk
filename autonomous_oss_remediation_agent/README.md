...
Each cycle is governed by an append-only Markdown journal at `artifacts/agent/decision-journal.md`. Read-only discovery (bounded file listing, text search, targeted reads, and fixed read-only Git inspection) precedes a metadata-only Problem Analysis and Solution Decision submission. Repository mutation and shell capabilities remain unavailable until the submission is structurally accepted. After execution, repository capabilities are removed and a metadata-only Cycle Outcome is captured before deterministic validation. Deterministic code renders headings, validates a deliberately limited Markdown subset and required sections, appends validation and final-resolution sections, and records hashes and lifecycle events. The checkpoints constrain capture timing and completeness, not engineering choices; one credible approach, reversible experiments, and evidence-driven strategy changes remain valid.

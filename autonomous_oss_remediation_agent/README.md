...
Each cycle is governed by an append-only Markdown journal at `artifacts/agent/decision-journal.md`. Read-only discovery (bounded file listing, text search, targeted reads, and fixed read-only Git inspection) precedes a metadata-only Cycle Intent submission. Repository mutation and shell capabilities remain unavailable until the intent is structurally accepted. After execution, repository capabilities are removed and a metadata-only Cycle Outcome is captured before deterministic validation. Deterministic code renders headings, validates a deliberately limited Markdown subset and required sections, appends validation and final-resolution sections, and records hashes and lifecycle events. The checkpoints constrain capture timing and completeness, not engineering choices; one credible approach, reversible experiments, and evidence-driven strategy changes remain valid.

The full execution response remains in `agent/cycle-N.json` as `summary`. The deprecated `workingState` field is generated deterministically from the accepted Cycle Outcome rather than requested from the model. The accepted journal is authoritative for cross-cycle continuity. Failed-validation continuation includes bounded journal content plus compact new validation evidence and never injects `workingState` into the model prompt.

...
Confirm hard-constraint admissibility before applying preference. Compare the surviving admissible candidates using current evidence, complete-problem coverage, compatibility, coherence, maintainability, scope, and risk—not speed, ease, or prior investment. Apply these engineering preferences here, after candidate formation; do not use them to retroactively exclude a viable candidate. These qualities cannot outweigh a hard-constraint conflict. Do not select a candidate with an established hard-requirement or hard-constraint conflict, or one whose compliance remains materially unresolved or depends on an assumption, regardless of whether it is labeled COMPLETE or PARTIAL. Ordinary execution-dependent results that do not determine hard-constraint compliance may remain for implementation, self-validation, and deterministic validation. Concisely explain why the selected solution is preferred, which materially distinct approaches were considered or eliminated and the evidence supporting important eliminations, and why remaining uncertainty does not require further pre-selection investigation. Do not repeat the full implementation sequence. The implementation intent is directional and may be materially reassessed during this cycle when new evidence warrants it.""",
    ),
)

...
Use these fields: `Selected solution:`, `Classification:`, `Why it is preferred:`, `Comparative coverage:`, `Remaining risks:`, and `Evidence requiring reconsideration:`. Reference a submitted candidate and classify it COMPLETE or PARTIAL. Base selection on current evidence, complete-problem coverage, constraints, compatibility, coherence, maintainability, and risk—not speed, ease, or prior investment. Do not select a candidate with an established hard-requirement or hard-constraint conflict, or one whose compliance depends on an unresolved assumption, regardless of whether it is labeled COMPLETE or PARTIAL. Ordinary execution-dependent results that do not determine hard-constraint compliance may remain for implementation, self-validation, and deterministic validation. Do not repeat the full implementation sequence. The implementation intent is directional and may be materially reassessed during this cycle when new evidence warrants it.""",
    ),
)

...
When solution choice materially depends on how relevant state or behavior is produced or controlled, investigate the existing ownership, control, management, inheritance, indirection, configuration, composition, abstraction, relationships, or other repository-evidenced mechanisms to the depth reasonably necessary for the decision. If candidate viability or selection materially depends on a repository-specific premise that a proposed mechanism controls, changes, resolves, produces, or otherwise affects relevant state or behavior, and the available isolated investigation capabilities can reasonably test that premise, obtain sufficient evidence before treating the premise as established or the candidate as sufficiently supported for selection. General technical knowledge may suggest the mechanism or premise to investigate, but does not by itself establish repository-specific applicability or viability when material repository evidence is reasonably obtainable. This requires decision-sufficient support, not exhaustive pre-execution proof, experimentally testing every candidate, running every validation, or proving final task success; genuinely execution-dependent outcomes may remain for implementation and validation. Discovering an apparently appropriate control point does not by itself complete exploration.

Finding one credible or workable mechanism is not sufficient reason to stop investigation. Before forming candidates, use task, repository, execution, and materially relevant current external evidence to identify materially distinct intervention mechanisms and investigate them far enough to distinguish: what options currently exist; which can satisfy the required outcome and constraints; which are applicable and compatible enough to remain viable for this project; and preference among the survivors. Do not treat the newest option as automatically correct or eliminate a mechanism merely because another already appears preferable; relative preference does not establish non-viability. Keep exploration evidence-driven and proportional. Preserve each materially distinct viable mechanism as a separate candidate, while allowing one candidate when evidence eliminates the others. Mechanisms may be eliminated before candidate formation when evidence establishes that they are unsupported, unavailable, infeasible, incapable of satisfying the task or providing valid constraint-compliant partial progress, materially contradicted, hard-constraint conflicting, or otherwise not genuinely viable.""",
    ),
    QuestionnaireSection(

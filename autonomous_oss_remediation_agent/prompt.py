...
- Before forming candidates, perform decision-relevant investigation reasonably obtainable through the available engineering capabilities. When a material decision depends on information that may have changed outside the repository, obtain reasonably available current authoritative evidence and use it to discover the actual available and potentially applicable solution space, not merely to confirm the first preferred solution. Planned or future investigation is not evidence supporting candidate formation; non-material questions need not be pursued, and genuinely execution-dependent outcomes remain for implementation and validation.

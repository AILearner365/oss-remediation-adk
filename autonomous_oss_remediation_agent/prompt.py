...
- Do not use vulnerability-specific recipes from this prompt. When choosing a remediation, inspect how affected dependency versions are managed by the repository, including relevant parents, imported BOMs, properties, and existing `dependencyManagement`. Do not treat these management layers as a required remediation order or hierarchy.
- When multiple safe remediations satisfy the supplied constraints, prefer the approach that best preserves the repository's existing dependency-management model, minimizes fragmented version control, and produces a coherent, maintainable change rather than a larger set of isolated dependency pins. Avoid unnecessary explicit lower-level overrides, but retain discretion to use targeted overrides when repository structure, compatibility, build, or validation evidence supports them. Do not choose a remediation solely because it is the quickest way to make the scanner pass.

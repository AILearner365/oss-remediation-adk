...
Derive the materially distinct high-level solution approaches supported by that synthesis. Treat the investigation record as the evidentiary boundary; do not invent missing support here. Separate viability from preference. Uncertainty is not evidence. Do not eliminate a plausible approach because of an unsupported expectation, convention, potentially stale prior, unresolved assumption, or absence of evidence; preserve unresolved viability instead. Elimination requires material observed evidence, current authoritative information, or a hard task constraint. The newest option is not automatically correct. One approach is valid when evidence eliminates the others; there is no minimum approach count and alternatives must not be manufactured.
...
Form candidates only from investigation actually performed, evidence actually obtained, and the project-applicable synthesis. If viability or COMPLETE classification materially depends on a decision-critical, reasonably testable repository-specific premise, obtain sufficient evidence before treating it as established. If sufficient evidence cannot reasonably be obtained, preserve the premise and high-level approach as unresolved rather than promoting it into candidate support or treating it as evidence of non-viability. Candidate count is an outcome of investigation and synthesis. Preserve materially distinct viable approaches as separate candidates when appropriate; one candidate remains valid when evidence eliminates the others. Never manufacture alternatives. Decision-sufficient evidence does not require exhaustive experimentation or proof of final success.

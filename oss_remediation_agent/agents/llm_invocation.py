...
class LLMAgentInvoker(Protocol):
    ...
    The orchestrator owns lifecycle and routing, but the AI agents own reasoning.
    This interface keeps LLM invocation isolated from deterministic tools and
    allows tests to inject fixed structured JSON outputs without turning agents
    into deterministic rule engines.
    """
...
class GoogleGenAIJsonInvoker:
    """Default JSON-only LLM invoker for ADK runtime environments.

    The import is intentionally lazy so unit tests and local compile checks do
    not require the Google GenAI SDK. In ADK/Gemini environments the SDK is
    typically available through installed ADK dependencies and credentials.
    """

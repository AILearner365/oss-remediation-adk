...
class Phase6PatchProgressOrchestrator(Phase6WorkflowOrchestrator):
    ...
    def _terminal_status(self, manifest: dict[str, Any] | None = None) -> str | None:
        manifest = manifest or self.manifest_store.load()
        status = str(manifest.get("status") or "").upper()
        return status if status in self._PRE_REMEDIATION_TERMINAL_STATUSES else None

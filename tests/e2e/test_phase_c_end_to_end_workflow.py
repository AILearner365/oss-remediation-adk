...
class PhaseCEndToEndWorkflowTests(unittest.TestCase):
    def test_e2e_successful_remediation_generates_eligible_pr_summary(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            plan_path = _write_patch_plan(Path(tmp), "patch-plan-success.json", "patch-success", "GHSA-success", "1.33", "2.2")
            with _mock_pre_remediation("SUCCESS"), _mock_attempt_lifecycle({1: "SUCCESS"}):
    ...
    def test_e2e_baseline_build_failure_stops_before_assessment_and_attempts(self):
        with tempfile.TemporaryDirectory() as tmp:
            orchestrator = WorkflowOrchestrator(tmp)
            with _mock_pre_remediation("BASELINE_FAILED"):
    ...
    def test_e2e_manual_review_required_generates_not_eligible_summary(self):
        with tempfile.TemporaryDirectory() as tmp:
            orchestrator = WorkflowOrchestrator(tmp)
            with _mock_pre_remediation("SUCCESS"):
                _run_pre_remediation(orchestrator)
                result = orchestrator.handle_planner_result({"decisionType": "MANUAL_REVIEW", "reason": "Unsupported migration"}, attempt_number=1)
            self.assertEqual(result["status"], "SUCCESS")
            manifest = _manifest(Path(tmp))
    ...
    def test_e2e_retry_then_success_uses_second_attempt_as_accepted_patch_set(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            first_plan = _write_patch_plan(Path(tmp), "patch-plan-attempt-1.json", "patch-attempt-1", "GHSA-retry", "1.33", "2.1")
            second_plan = _write_patch_plan(Path(tmp), "patch-plan-attempt-2.json", "patch-attempt-2", "GHSA-retry", "1.33", "2.2")
            with _mock_pre_remediation("SUCCESS"), _mock_attempt_lifecycle({1: "FAILED", 2: "SUCCESS"}, failed_stage="OSV_VALIDATION"):
                _run_pre_remediation(orchestrator)
            ...
            self.assertEqual(_read_json(Path(tmp) / "final" / "pr-summary.json")["status"], "ELIGIBLE")
    ...
    def test_e2e_max_attempts_reached_without_accepted_patch_set(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            plans = [str(_write_patch_plan(Path(tmp), f"patch-plan-{i}.json", f"patch-{i}", "GHSA-max", "1.33", f"2.{i}")) for i in range(1, 4)]
            with _mock_pre_remediation("SUCCESS"), _mock_attempt_lifecycle({1: "FAILED", 2: "FAILED", 3: "FAILED"}, failed_stage="BUILD_VALIDATION"):
                _run_pre_remediation(orchestrator)
    ...
    def test_e2e_unsafe_change_scope_is_rejected_and_classified(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            plan = _write_patch_plan(Path(tmp), "patch-plan-unsafe.json", "patch-unsafe", "GHSA-unsafe", "1.33", "2.2", file_name="src/main/java/App.java")
            with _mock_pre_remediation("SUCCESS"), _mock_attempt_lifecycle({1: "FAILED"}, failed_stage="CHANGE_SCOPE_VALIDATION"):
                _run_pre_remediation(orchestrator)
                result = orchestrator.run_attempt_loop([str(plan)])
            self.assertEqual(result["status"], "FAILED")
            self.assertEqual(_manifest(Path(tmp))["acceptedPatchSet"]["status"], "EMPTY")
    ...
    def test_e2e_new_critical_or_high_vulnerability_introduced_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            orchestrator = WorkflowOrchestrator(tmp, policy=RemediationPolicy(max_attempts=1))
            plan = _write_patch_plan(Path(tmp), "patch-plan-new-high.json", "patch-new-high", "GHSA-new-high", "1.33", "2.2")
            with _mock_pre_remediation("SUCCESS"), _mock_attempt_lifecycle({1: "FAILED"}, failed_stage="OSV_VALIDATION", new_critical_high=True):
                _run_pre_remediation(orchestrator)
                result = orchestrator.run_attempt_loop([str(plan)])
            self.assertEqual(result["status"], "FAILED")
            self.assertEqual(_manifest(Path(tmp))["acceptedPatchSet"]["status"], "EMPTY")
...
def _run_pre_remediation(orchestrator: WorkflowOrchestrator) -> None:
    assert orchestrator.checkout_and_baseline("https://example.com/repo.git", "main")["status"] == "SUCCESS"
    assert orchestrator.run_assessment()["status"] == "SUCCESS"
    assert orchestrator.run_project_analysis()["status"] == "SUCCESS"


def _mock_pre_remediation(status: str):
    class Context:
        def __enter__(self):
    ...
    class Context:
        def __enter__(self):
            self.patchers = [
                ...
                patch("oss_remediation_agent.workflow.orchestrator.validate_attempt", side_effect=lambda **kwargs: _validation_result(kwargs, validation_status_by_attempt, failed_stage, new_critical_high)),
            ]
            for patcher in self.patchers:
                patcher.start()
            return self
...
def _write_assessment_result(repository_path: str, output_path: str, raw_report_path: str, severity_scope: str, workflow_id: str) -> dict:
    artifact = {"schemaVersion": "1.0", "artifactId": "vulnerability-assessment-report", "workflowId": workflow_id, "createdBy": "E2ETest", "status": "SUCCESS", "vulnerabilities": [{"vulnerabilityId": "GHSA-test", "severity": "HIGH", "dependency": "org.yaml:snakeyaml"}]}
    Path(output_path).parent.mkdir(parents=True, exist_ok=True)
...
def _write_project_analysis_result(repository_path: str, vulnerability_assessment_path: str, output_path: str, artifact_output_dir: str, workflow_id: str) -> dict:
    artifact = {"schemaVersion": "1.0", "artifactId": "project-analyzer-report", "workflowId": workflow_id, "createdBy": "E2ETest", "status": "SUCCESS", "projectFacts": {"projectType": "MAVEN", "rootPom": "pom.xml", "pomFiles": ["pom.xml"]}, "dependencyResolutionEvidence": [{"dependency": "org.yaml:snakeyaml", "resolvedVersion": "1.33"}]}
    Path(artifact_output_dir).mkdir(parents=True, exist_ok=True)
    Path(output_path).write_text(json.dumps(artifact), encoding="utf-8")
    return {"status": "SUCCESS", "artifactPath": output_path, "payload": artifact}
...
def _restore_attempt_workspace(workspace_root: str, attempt_number: int):
    ...
    app_dir = attempt_workspace / "src" / "main" / "java"
    app_dir.mkdir(parents=True, exist_ok=True)
    (app_dir / "App.java").write_text("class App {}", encoding="utf-8")
...
def _apply_success(attempt_number: int, repository_path: str, patch_plan_path: str, output_path: str, diff_path: str):
    Path(output_path).parent.mkdir(parents=True, exist_ok=True)
    patch_item = _read_json(Path(patch_plan_path))["vulnerabilityDecisions"][0]["patches"][0]
    artifact = {"schemaVersion": "1.0", "artifactId": f"patch-application-proof-{attempt_number}", "workflowId": "wf-e2e", "createdBy": "E2ETest", "status": "SUCCESS", "attemptNumber": attempt_number, "filesChanged": [patch_item["file"]], "patchResults": [{"patchId": patch_item["patchId"], "status": "APPLIED", "file": patch_item["file"]}]}
    Path(output_path).write_text(json.dumps(artifact), encoding="utf-8")
...
def _validation_result(kwargs: dict, validation_status_by_attempt: dict[int, str], failed_stage: str, new_critical_high: bool):
    ...
    artifact = {"schemaVersion": "1.0", "artifactId": f"validation-result-{attempt_number}", "workflowId": kwargs.get("workflow_id", "wf-e2e"), "createdBy": "E2ETest", "status": status, "summary": {"failedStage": None if status == "SUCCESS" else failed_stage, "failureSummary": None if status == "SUCCESS" else f"Intentional {failed_stage} failure for end-to-end workflow test."}, "changeScopeValidation": {"status": "SUCCESS" if status == "SUCCESS" or failed_stage != "CHANGE_SCOPE_VALIDATION" else "FAILED"}, "buildValidation": {"status": "SUCCESS" if status == "SUCCESS" or failed_stage != "BUILD_VALIDATION" else "FAILED"}, "testValidation": {"status": "SUCCESS" if status == "SUCCESS" or failed_stage != "TEST_VALIDATION" else "FAILED"}, "osvValidation": {"status": "SUCCESS" if status == "SUCCESS" else "FAILED", "remainingCriticalCount": 0, "remainingHighCount": 0 if status == "SUCCESS" else 1, "newCriticalHighIntroduced": bool(new_critical_high)}}
...
def _write_patch_plan(root: Path, filename: str, patch_id: str, vulnerability_id: str, old_version: str, new_version: str, file_name: str = "pom.xml") -> Path:
    path = root / filename
    path.write_text(json.dumps({"schemaVersion": "1.0", "artifactId": path.stem, "workflowId": "wf-e2e", "createdBy": "E2ETest", "status": "PATCH_AVAILABLE", "planId": path.stem, "vulnerabilityDecisions": [{"vulnerabilityId": vulnerability_id, "decision": "PATCH", "dependency": {"packageName": "org.yaml:snakeyaml", "currentVersion": old_version}, "patches": [{"patchId": patch_id, "file": file_name, "oldText": f"<snakeyaml.version>{old_version}</snakeyaml.version>", "newText": f"<snakeyaml.version>{new_version}</snakeyaml.version>", "oldVersion": old_version, "newVersion": new_version, "expectedOccurrences": 1}]}]}), encoding="utf-8")
    return path


def _patch_id(patch_plan_path: str) -> str:
    return _read_json(Path(patch_plan_path))["vulnerabilityDecisions"][0]["patches"][0]["patchId"]
